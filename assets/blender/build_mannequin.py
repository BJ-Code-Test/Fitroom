...
TORSO = [
    ...
    (1.440, 0.1400, 0.0940, 0.0980, 2.30, 0.0, 0.002),
    (1.460, 0.1160, 0.0850, 0.0900, 2.24, 0.0, 0.004),
    (1.480, 0.0860, 0.0710, 0.0790, 2.16, 0.0, 0.007),
    (1.500, 0.0665, 0.0590, 0.0700, 2.10, 0.0, 0.010),
    (1.515, 0.0575, 0.0520, 0.0670, 2.07, 0.0, 0.012),
...
# ------------------------------------------------------------ leg profile
LEG = [
    (0.000, 0.0480, 0.150, 0.107, 4.00, -0.104, -0.051),
    (0.008, 0.0480, 0.150, 0.103, 3.80, -0.104, -0.048),
    (0.020, 0.0470, 0.146, 0.100, 3.40, -0.104, -0.045),
...
ARM_ATTACH = (0.161, 0.0, 1.353)
ARM_LEN = 0.741
ARM_ANGLE = math.radians(77.0)   # rotation from T-pose -> 13 deg A-pose
ARM_FALLOFF = 0.105

ARM = [
    (0.000, 0.0390, 0.0625, 0.0620, 2.24, 0.0, 0.0),
    (0.025, 0.0455, 0.0610, 0.0600, 2.18, 0.0, 0.0),
    (0.055, 0.0505, 0.0578, 0.0568, 2.13, 0.0, 0.0),
    (0.090, 0.0525, 0.0545, 0.0538, 2.10, 0.0, 0.0),
    (0.130, 0.0520, 0.0520, 0.0516, 2.10, 0.0, 0.0),
    (0.200, 0.0495, 0.0495, 0.0495, 2.10, 0.0, 0.0),
    ...
    (0.810, 0.0235, 0.038, 0.032, 2.20, 0.0, 0.0),
    (0.870, 0.0225, 0.045, 0.036, 2.25, 0.0, 0.0),
    (0.930, 0.0215, 0.044, 0.035, 2.30, 0.0, 0.0),
    (0.970, 0.0185, 0.038, 0.031, 2.30, 0.0, 0.0),
    (1.000, 0.0130, 0.024, 0.020, 2.25, 0.0, 0.0),
]
P_ARM = Profile(ARM)
...
# I_TOP is the *boundary* ring of the arm socket, deliberately placed on the
# shoulder measurement loop so that loop stays closed.
I_TOP = min(range(len(ZS_T)), key=lambda k: abs(ZS_T[k] - Z_SHOULDER))
I_BOT = I_TOP - 6
J0_R = N_T // 4 - 3          # 9
J1_R = N_T // 4 + 3          # 15
...
taubin({i: max(0.15, 1.0 - 0.14 * d) for i, d in dist.items()}, passes=6)
